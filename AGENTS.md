...
По умолчанию только видимая главная сессия является root orchestrator для задачи пользователя и запускает subagents. Spawned/launched agent is scoped executor/reviewer by default, even if role says ops/orchestrator: название роли не даёт права на вложенную делегацию.

Это не запрет оркестрации: видимый root orchestrator может вести stages/gates для крупных задач: разведка -> реализация -> независимое ревью -> доработка -> проверка -> коммит/деплой по правилам проекта -> production/data-сверка -> статус-документ.

Запущенный агент не запускает nested subagents, hidden Codex sessions или `codex exec` как слой делегирования без прямого разрешения в parent brief. Если ему нужен ещё один исполнитель, он возвращает запрос главному видимому оркестратору. Если parent brief прямо назначил агента локальным оркестратором и разрешил nested delegation, агент обязан явно отчитаться: кого/что запустил, как, зачем, границы scope и evidence.
