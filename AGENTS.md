...
Жёсткое правило: оркестратор не делает исполнительские правки руками. Его ручная работа: понять задачу, прочитать нужный контекст, написать brief, запустить и скоординировать агентов, проверить diff/evidence, принять результат и собрать финальный ответ. Даже маленькие файловые правки делает профильный агент-исполнитель. Если subagents/runtime заблокированы, оркестратор прямо сообщает о блокере и фиксирует исключение или ждёт решения пользователя, а не молча правит сам.
